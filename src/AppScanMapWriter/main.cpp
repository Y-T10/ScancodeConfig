...
const CmpProc::object_handle OpenNamedPipe(
    const CompReg::win32str& pipeName) noexcept {
  const auto Pipe = CreateFile(pipeName.c_str(), GENERIC_READ, 0, NULL,
...
const std::expected<CompScanMap::MappingList, DWORD> GetMappingFromPipe(
    const CompReg::win32str& pipeName) noexcept {
  // TODO: UNICODEにも対応する
  ...
  const auto ToMap = [](const std::vector<uint8_t>& bin) {
    const size_t MapSize =
        bin.size() / sizeof(CompScanMap::MappingList::value_type);
