...
const char8_t ToAcceptedChar(const char8_t c) noexcept {
  ...
  const auto RplaceOutOfRangeChar = [](const char8_t c) -> char8_t {
    ...
    // 文字を' '以上にする
    return SingleChar <= u8' ' ? SingleChar + 33 : SingleChar;
  };
...
const std::expected<object_handle, DWORD> CreatePipe(
    const CompReg::win32str& name, const size_t bufferSize) noexcept {
  const DWORD PipeMode = PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT |
                         PIPE_REJECT_REMOTE_CLIENTS;
  const auto Handle =
      CreateNamedPipe(name.c_str(), PIPE_ACCESS_OUTBOUND, PipeMode, 1,
...
const std::expected<void, DWORD> WritePipe(
    const object_handle& handle, const std::vector<uint8_t>& byte) noexcept {
  DWORD writtenSize = 0;
  if (WriteFile(handle.get(), byte.data(), byte.size(), &writtenSize,
                nullptr)) {
...
const std::expected<std::vector<uint8_t>, DWORD> ReadPipe(
