...
namespace {
#ifdef _WIN32
using HToken = std::unique_ptr<std::remove_pointer_t<HANDLE>,
...
const bool SetTokenPriv(const HToken& token, const TCHAR* privName) {
  ...
  // トークンの特権を設定し、その結果を返す
  if (!AdjustTokenPrivileges(token.get(), FALSE, (TOKEN_PRIVILEGES*)&TokePriv,
...
const std::optional<bool> IsElevated() noexcept {
  ...
  if (0 == GetTokenInformation(token.get(),
                               TOKEN_INFORMATION_CLASS::TokenElevation,
                               &evelationInfo, size, &size)) {
    return std::nullopt;
  }
