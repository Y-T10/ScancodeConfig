...
const std::expected<std::wstring, int> ACPToUTF16(
    const std::string& str) noexcept {
  if (str.empty()) {
    return L"";
  }
  const int size =
      MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, str.c_str(), -1, nullptr, 0);
  if (!size) {
...
const std::expected<std::string, int> UTF16To8(
    const std::wstring& str) noexcept {
  if (str.empty()) {
    return "";
  }
  const int size = WideCharToMultiByte(CP_UTF8, 0, str.c_str(), -1, nullptr, 0,
                                       nullptr, nullptr);
  if (!size) {
    return std::unexpected<int>(GetLastError());
  }
  std::vector<char> output(size, '\0');
  if (!WideCharToMultiByte(CP_UTF8, 0, str.c_str(), -1, output.data(),
...
const std::expected<std::string, int> ToUTF8(
  ...
  if constexpr (std::is_same_v<char_type, wchar_t>) {
    ...
    return ACPToUTF16(str).and_then(&UTF16To8);
  }
}
}  // namespace

namespace CompScanMap {
const std::optional<std::string> WindowsScancodeName(
    const Scancode code) noexcept {
...
const std::expected<std::string, int> KeyboardKeyName(
    const Scancode code) noexcept {
  ...
  const int NameSize = GetKeyNameText(MakeLParam(code), keyName, keyNameLength);
