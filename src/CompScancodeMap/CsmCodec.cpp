...
const integer_type Byteswap(const integer_type value) noexcept {
  ...
  if constexpr (std::is_same_v<integer_type, uint8_t>) {
    ...
    static_assert([]() { return false; },
                  "this integer type is not supported.");
  }
}
...
const bool IsVailedScanMapBin(const std::vector<uint8_t>& bin) noexcept {
  const auto isZero = [](const uint8_t value) -> bool { return value == 0x00; };
...
const std::optional<MappingList> DecodeScancodeMap(
    const std::vector<uint8_t>& bin) noexcept {
  ...
  if constexpr (std::endian::native == std::endian::big) {
    std::transform(map.begin(), map.end(), map.begin(),
                   [](const ScanMapping& origin) {
                     return ScanMapping{.to = Byteswap(origin.to),
                                        .from = Byteswap(origin.from)};
                   });
...
const std::optional<std::vector<uint8_t>> EncodeScancodeMap(
    const MappingList& map) noexcept {
  // 表が空かを調べる
  if (map.empty()) {
  ...
  if constexpr (std::endian::native == std::endian::big) {
    for (auto valHead = (scanMapBin.begin() + 12); valHead != scanMapBin.end();
         valHead += sizeof(Scancode)) {
